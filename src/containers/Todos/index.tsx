...
const Todos: React.FC = () => {
    ...
    //Add Category
    const [isAddCategoriesLoading, setIsAddCategoriesLoading] = React.useState<boolean>(false);
    const addCategory = async (category: CategoryCreationProps): Promise<void> => {
        try {
            setIsAddCategoriesLoading(true);
            await dispatch(addCategoryAction(category));
            setIsAddCategoriesLoading(false);
        } catch (error) {
        ...
        <React.Fragment>
            ...
            {!(isTodosFetchFailed || isCategoriesFetchFailed) &&
                <AddTodo
                    ...
                    isAddCategoryLoading={isAddCategoriesLoading}
                    onAddTodo={addTodo}
